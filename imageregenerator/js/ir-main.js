function query_regenerate_img(e) {
    $("input[name='image_regenerator_queue_what']").val(e), image_regenerator_can_run_queue ? $.ajax({
        ...
        dataType: "json",
        success: function(a) {
            1 == a.success ? ($("#autoImg-progress").prepend(["<p>Success : ", e, " ", autoImg[e].todo[0], "</p>"].join("")), autoImg[e].done.push(autoImg[e].todo[0]), autoImg[e].todo.shift()) : ($("#autoImg-progress").prepend(["<p>error : ", e, " ", autoImg[e].todo[0], "</p>"].join("")), autoImg[e].errors.push(autoImg[e].todo[0]), autoImg[e].todo.shift()), $(["#image_regenerator-infos-", e].join("")).find(".image_regenerator-done").html(autoImg[e].done.length), autoImg[e].todo.length > 0 ? (query_regenerate_img(e), progress = Math.round(100 * (autoImg[e].done.length / (autoImg[e].done.length + autoImg[e].todo.length)), 2), $(["#image_regenerator-infos-", e].join("")).find(".progress-bar").css("width", [progress, "%"].join(""))) : ($("#autoImg-progress").prepend(["<p>Finished : ", e, "</p>"].join("")), $(".image_regenerator-lauch").removeAttr("disabled"), $("#image_regenerator-save").removeAttr("disabled"), $(["#image_regenerator-infos-", e].join("")).find(".progress-bar").css("width", "100%"))
        }
...
jQuery(document).ready(function(e) {
    ...
    e.each(autoImg, function(r, t) {
        a = Math.round(100 * (t.done.length / (t.todo.length + t.done.length)), 2), e("#autoImg-buttons").append(["<tr id='image_regenerator-infos-", r, "'><td><button class='image_regenerator-lauch btn btn-info' data-what='", r, "'><span class='icon-play'></span> Regenerate ", r, "</button></td><td><span class='image_regenerator-done'>", t.done.length, "</span>/<span class='image_regenerator-remaining-norefresh'>", t.done.length + t.todo.length, "</span></td><td width='50%'><div class='progress'><div class='progress-bar' role='progressbar' aria-valuenow='", a, "' aria-valuemin='0' aria-valuemax='100' style='width: ", a, "%;'><span class='sr-only'>", a, "% Complete</span></div></div></td></tr>"].join(""))
